import { Renderer, Triangle, Program, Mesh } from './lib/ogl.js';
...
export function createPrism(container, options = {}) {
  ...
  const onMove = e => {
    ...
    const nx = (e.clientX - cx) / (rect.width * 0.5 || 1);
    const ny = (e.clientY - cy) / (rect.height * 0.5 || 1);
    pointer.x = Math.max(-1.5, Math.min(1.5, nx));
    pointer.y = Math.max(-1.5, Math.min(1.5, ny));
    pointer.inside = true;
  };
  const onLeave = () => {
